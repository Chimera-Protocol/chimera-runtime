"""
chimera-compliance CLI — Main entry point

Usage:
    chimera-compliance --help
    chimera-compliance init
    chimera-compliance run [--daemon] [--require-confirmation]
    chimera-compliance stop [--force]
    chimera-compliance verify [POLICY]
    chimera-compliance audit [--last N] [--result RESULT] [--stats]
    chimera-compliance policy [new|list|simulate]
    chimera-compliance explain --id <decision_id>
...
@click.group()
@click.version_option(version=__version__, prog_name="chimera-compliance")
@click.option("--config", "-c", default=".chimera/config.yaml",
              help="Path to config file", show_default=True)
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
@click.pass_context
def cli(ctx, config, verbose):
    """🛡️ chimera-compliance — EU AI Act Compliant Agent Framework
