"""chimera-compliance run — Start the agent in interactive or daemon mode."""
...
from chimera_compliance import AgentHalted, ChimeraAgentError
...
def run_cmd(ctx, daemon, require_confirmation, human_override, dry_run, model, policy):
    ...
    try:
        ...
        err_console.print("[dim]Run 'chimera-compliance init' to create a config file.[/dim]")
        raise SystemExit(1)
...
def _run_interactive(agent, ctx, dry_run=False):
    ...
    while True:
        ...
        if request.lower() == "halt":
            agent.halt()
            console.print("[bold red]🛑 Agent HALTED.[/bold red] [dim]Use 'chimera-compliance run' to restart.[/dim]")
            break
...
def _run_daemon(agent, ctx):
    err_console.print("[dim]🤖 chimera-compliance daemon mode. Send JSON to stdin.[/dim]")
