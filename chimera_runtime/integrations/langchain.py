"""
chimera-runtime — LangChain Integration

Provides:
  - ChimeraRuntimeTool: Wraps any LangChain tool with runtime guard
  - ChimeraCallbackHandler: Intercepts tool calls as a callback handler
  - wrap_tools(): Convenience function to wrap multiple tools

Install: pip install chimera-runtime[langchain]
...
def _require_langchain() -> None:
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            ...
            "Install it with: pip install chimera-runtime[langchain]"
        )


class ChimeraRuntimeTool(ActionGuardMixin):
    """
    Wraps a LangChain BaseTool with a runtime guard.
    ...
    def __init__(
        ...
        def guarded_run(*args: Any, **kwargs: Any) -> Any:
            ...
            if evaluation.result == "BLOCKED":
                ...
                raise ToolException(
                    f"[chimera-runtime] Tool '{tool.name}' BLOCKED: {violations}"
                )
...
def wrap_tools(
    ...
    """
    Wrap a list of LangChain tools with runtime guards.
    ...
    for tool in tools:
        ChimeraRuntimeTool(tool, guard, param_extractor)
