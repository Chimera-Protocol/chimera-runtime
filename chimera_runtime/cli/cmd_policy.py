"""chimera-runtime policy — Policy management commands."""
...
'''
    ...
    console.print(f"  [dim]Next: Edit the file, then run [bold]chimera-runtime verify {filepath}[/bold][/dim]\n")
...
def policy_simulate(ctx, policy_file, context_json, input_file, dry_run):
    ...
    \b
    Examples:
        chimera-runtime policy simulate policies/gov.csl '{"amount": 50000}'
        chimera-runtime policy simulate policies/gov.csl --input test_cases.json
    """
    from ..policy import PolicyManager, PolicyError
