"""chimera-runtime init — Project initialization wizard."""
...
MODES = {
    1: ("standalone", "Standalone Agent — chimera-runtime manages the LLM directly"),
...
def init_cmd(ctx: CLIContext, non_interactive: bool, mode: str):
    ...
    \b
      standalone   — chimera-runtime manages the LLM directly
    ...
    if mode is None and not non_interactive:
        ...
        console.print("[bold]How will you use chimera-runtime?[/bold]")
    ...
    config = AgentConfig(
        agent=AgentMetaConfig(name="chimera-runtime", version="0.1.0"),
        llm=llm_config,
    ...
    if mode == "standalone":
        steps = [
            ...
            f"Verify it: [cyan]chimera-runtime verify {policy_file}[/cyan]",
            "Run agent: [cyan]chimera-runtime run[/cyan]",
        ]
    else:
        from rich.markup import escape
        install_cmd = escape(f"pip install chimera-runtime[{framework}]")
        steps = [
            f"Edit your policy: [magenta]{policy_file}[/magenta]",
            f"Test it: [cyan]chimera-runtime test --skip-llm[/cyan]",
...
def _integration_snippet(framework: str, policy_file: str) -> str:
    ...
    snippets = {
        "langchain": (
            "Use in code: [dim]from chimera_runtime.integrations.langchain import wrap_tools[/dim]"
        ),
        "langgraph": (
            "Use in code: [dim]from chimera_runtime.integrations.langgraph import compliance_node[/dim]"
        ),
        "crewai": (
            "Use in code: [dim]from chimera_runtime.integrations.crewai import wrap_crew_tools[/dim]"
        ),
        "llamaindex": (
            "Use in code: [dim]from chimera_runtime.integrations.llamaindex import wrap_tools[/dim]"
        ),
        "autogen": (
            "Use in code: [dim]from chimera_runtime.integrations.autogen import guard_function_call[/dim]"
        ),
    }
